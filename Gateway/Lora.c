...
/**
 * @brief 
 * This function is to check the message from the device
 * @param Lora_sensor_message                   the message that received from the device
 * @param flag                                  the flag for successful message reception      
 * @param Sensor_message_len                    message length              
 * @return                                      the check result
 */
int Lora_message_check(char* Lora_sensor_message,int flag, int Sensor_message_len){
    int Check_Byte = 0;
    if(flag!=1)     
        return 4;
    ...
    if(Check_Byte!=Lora_sensor_message[25])
        return 2;
    
    if((Lora_sensor_message[0]!=0xFE) || (Lora_sensor_message[1]!=0xFE) || (Lora_sensor_message[Sensor_message_len-2]!=0xEF) || (Lora_sensor_message[Sensor_message_len-1]!=0xEF))
        return 3;    
